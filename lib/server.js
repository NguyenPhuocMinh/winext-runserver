  ...
  this.startup = function () {
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
        ...
        console.log(chalk.green.underline('The server is running on %s://%s:%s', protocol, host, port));
        ...
        if (!isEmpty(swaggerOptions)) {
          loggerTracer.info('Connected swagger options successfully!');
          const swaggerDocs = swaggerJsDoc(swaggerOptions);
          loggerTracer.fatal(`Swagger info: ${JSON.stringify(swaggerDocs.info, null, 1)}`);
          app.use('/api-docs', swaggerUI.serve, swaggerUI.setup(swaggerDocs));
        } else {
          loggerTracer.warn('Not connect swagger options');
        }
      });
