...
function Server(params = {}) {
  ...
  const keyManager = get(params, 'keyManager');
  ...
  const keyManagerConfig = get(params, 'keyManagerConfig');
  ...
  const clientUIPath = process.env.CLIENT_PATH || get(config, 'clientUIPath');
  ...
  const clientKeyStorePath = process.env.KEY_STORE_PATH || get(keyManagerConfig, 'clientKeyStorePath');
  ...
  app.use(cors({ credentials: true, origin: [clientUIPath, clientKeyStorePath] }));
  app.use(morgan('dev'));
  ...
  this.startServer = function () {
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
         ...
         * Key store
         */
        app.use(contextPath, (request, response, next) =>
          keyManager.middleware({ app, router, request, response, next })
        );
