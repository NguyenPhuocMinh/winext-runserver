...
function Server(params = {}) {
  ...
  this.startup = function () {
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
        ...
        loggerTracer.warn(`Load ${name}-${version} successfully!`)
        const port = serverInstance.address().port;
        const host = serverInstance.address().address;
        loggerTracer.info(`The server is running on ${protocol}://${host}:${port}`)
      ...
      .then(info => {
        Server.call(
          isFunction(repository.startMongoose()) && repository.startMongoose(),
          isFunction(repository.startMySql()) && repository.startMySql(),
          isFunction(mappingStore.mapping) && mappingStore.mapping(app, router)
        );
        return info;
      })
      .catch(err => {
  ...
  this.shutdown = function () {
    return new Promise(function (resolve, reject) {
      ...
      .then(() => {
        Server.call(
          isFunction(repository.closeMongoose()) && repository.closeMongoose(),
          isFunction(repository.closeMySql()) && repository.closeMySql()
        );
        process.exit(0);
      })
      .catch(err => {
