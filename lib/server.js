  ...
  this.startServer = function () {
    ...
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
        if (err) {
          loggerTracer.error(`Start server has error :${err}`, {
            args: { err },
          });
          reject(err);
        }
          ...
          switch (dialectSwagger) {
            case options.swaggerType.options: {
              if (isEmpty(swaggerOptions)) {
                loggerTracer.warn('Load swagger options fail!');
                ...
                loggerTracer.info(`Api docs path: ${pathServer}${pathDocs}`);
              }
              break;
            }
            case options.swaggerType.yaml:
              if (isEmpty(swaggerYaml)) {
                loggerTracer.warn('Load swagger yaml fail!');
              ...
              break;
            default:
              loggerTracer.warn('Load swagger fail!');
        ...
        if (isEmpty(gatewayConfig) || !enableKongGateWay) {
          loggerTracer.warn('Load api gateway fail!');
        } else {
          app.use(
            contextPath,
        ...
        if (isEmpty(serviceRegistry) || !enableConsulRegistry) {
          loggerTracer.warn('Load service registry fail!');
        } else {
      ...
      .then((info) => {
        ...
        loggerTracer.info(`ContextPath: ${contextPath}`);
        loggerTracer.info(`The server is running on ${pathServer}${contextPath}`);
