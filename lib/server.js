    ...
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
        ...
        if (!isEmpty(swaggerOptions)) {
          ...
          loggerTracer.warn(chalk.hex('#70352d').bold('Load swagger options fail!'));
        }
        /**
         * Api gateway
         */
        ...
        if (isEmpty(gatewayConfig) || !enableKongGateWay) {
          loggerTracer.warn(chalk.hex('#70352d').bold('Load api gateway fail!'));
        } else {
          app.use(
            contextPath,
        ...
        if (isEmpty(serviceRegistry) || !enableConsulRegistry) {
          loggerTracer.warn(chalk.hex('#70352d').bold('Load service registry fail!'));
        } else {
