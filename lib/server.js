...
function Server(params = {}) {
  ...
  this.startServer = async function () {
    try {
      ...
      const serverInstance = server.listen(portServer, hostServer, (err) => {
          ...
          switch (dialectSwagger) {
            case options.swaggerType.options: {
              if (isEmpty(swaggerOptions)) {
                ...
                loggerTracer.info('Swagger options has been start !!!');
                const swaggerDocs = swaggerJsDoc(swaggerOptions);
                app.use(pathDocs, swaggerUI.serve, swaggerUI.setup(swaggerDocs));
                loggerTracer.http(`Docs path`, {
            ...
            case options.swaggerType.yaml:
              if (isEmpty(swaggerYaml)) {
                ...
                loggerTracer.info('Swagger yaml has been start !!!');
        ...
        if (isEmpty(gatewayConfig)) {
          loggerTracer.silly('Api gateway config is null!');
        } else if (!enableKongGateWay) {
          loggerTracer.silly('Not enable api gateway!');
        } else {
          app.use(
            contextPath,
        ...
        if (isEmpty(serviceRegistryConfig)) {
          loggerTracer.silly('Service registry config is null!');
        } else if (!enableConsulRegistry) {
          loggerTracer.silly('Not enable service registry!');
        } else {
        ...
        loggerTracer.verbose(`Rest path`, {
          args: `[${contextPath}]`,
        });
  ...
  this.stopServer = function () {
    return new Promise(function (resolve, reject) {
      ...
      loggerTracer.verbose(`Shutdown server timeout ${timeOut} ms`);
