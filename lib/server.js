...
function Server(params = {}) {
    ...
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
        ...
        if (!isEmpty(swaggerOptions)) {
          loggerTracer.info(chalk.green('Load swagger options successfully!'));
          const swaggerDocs = swaggerJsDoc(swaggerOptions);
          app.use(contextPath, '/api-docs', swaggerUI.serve, swaggerUI.setup(swaggerDocs));
          loggerTracer.info(chalk.green(`Api docs path: ${protocol}://${host}:${port}${contextPath}/api-docs`));
        } else {
          loggerTracer.warn(chalk.green('Load swagger options fail!'));
        }
        /**
         * Api gateway
         */
        ...
        if (isEmpty(gatewayConfig) || !enableKongGateWay) {
          loggerTracer.warn(chalk.green('Load api gateway fail!'));
        } else {
          app.use(
            contextPath,
        ...
        if (isEmpty(serviceRegistry) || !enableConsulRegistry) {
          loggerTracer.warn(chalk.green('Load service registry fail!'));
        } else {
  ...
  this.stopServer = function () {
    return new Promise(function (resolve, reject) {
      ...
      loggerTracer.info(chalk.info(`Shutdown timeout ${timeOut}`));
      ...
      .then(() => {
        Server.call(repoStore.stopMongo(), repoStore.closeMySql(), redisStore.stopRedis());
      })
      .then(() => {
        process.exit(0);
      })
