...
function Server(params = {}) {
  ...
  this.startup = function () {
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
        ...
        loggerTracer.info(chalk.yellow(`Load ${name}-${version} successfully!`));
        const port = serverInstance.address().port;
        const host = serverInstance.address().address;
        console.log(chalk.cyan.underline(`The server is running on, ${protocol}://${host}:${port}`));
        ...
        if (!isEmpty(swaggerOptions)) {
          loggerTracer.info(chalk.yellow('Connected swagger options successfully!'));
          const swaggerDocs = swaggerJsDoc(swaggerOptions);
          loggerTracer.fatal(chalk.cyan(`Swagger info: ${JSON.stringify(swaggerDocs.info, null, 1)}`));
  ...
  this.shutdown = function () {
    return new Promise(function (resolve, reject) {
      ...
      loggerTracer.debug(chalk.yellow(`Shutdown time out ${timeOut}`));
