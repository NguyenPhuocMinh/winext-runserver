...
const log4js = winext.require('log4js');
  ...
  this.startup = function () {
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
        ...
        const logTracer = log4js.getLogger('app-runserver');
        logTracer.tracer('The server is running on %s://%s:%s', protocol, host, port);
      ...
      .then(info => {
        Server.call(repository.startMongoose(), repository.startMySql(), mappingStore.mapping(app, router));
        return info;
      })
      .catch(err => {
        loggerFactory.error(`Start server has error : ${err}`, {
