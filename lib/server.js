  ...
  this.startServer = function () {
    return new Promise((resolve, reject) => {
      const serverInstance = server.listen(portServer, hostServer, (err) => {
        ...
        loggerTracer.info(chalk.green.bold(`Load ${name}-${version} successfully!`));
        ...
        if (!isEmpty(swaggerOptions)) {
          ...
          loggerTracer.warn(chalk.hex('#eded21').bold('Load swagger options fail!'));
        }
        /**
         * Api gateway
         */
        ...
        if (isEmpty(gatewayConfig) || !enableKongGateWay) {
          loggerTracer.warn(chalk.hex('#eded21').bold('Load api gateway fail!'));
        } else {
          app.use(
            contextPath,
        ...
        if (isEmpty(serviceRegistry) || !enableConsulRegistry) {
          loggerTracer.warn(chalk.hex('#eded21').bold('Load service registry fail!'));
        } else {
